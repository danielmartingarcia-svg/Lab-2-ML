...
for level in datasets:
    ...
    # Precompute P matrices for different kernels
    kernels = [
        (f.linearKernel, (), "Linear"),
        (f.polynomialKernel, (3,), "Polynomial (p=3)"),
        (f.rbfKernel, (1.0,), "RBF (sigma=1.0)")
    ]

    plt.figure(figsize=(18, 7))

    for idx, (k_func, k_args, name) in enumerate(kernels):
    ...
    plt.suptitle("Task 2: Comparison of Different Kernels for the "+ level +" dataset", fontsize=16)
    plt.tight_layout()
    plt.show()